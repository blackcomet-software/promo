...
export const task = pgTable("task", {
  ...
  pgPolicy("Allow project members to edit task", {
    as: "permissive", for: "update", to: ["authenticated"], using: sql`(( SELECT auth.uid() AS uid) IN ( SELECT project_member.user_id
   FROM project_member
...
export const inviteToProject = pgTable("invite_to_project", {
  ...
  pgPolicy("Allow any project member to insert a invite", {
    as: "permissive", for: "insert", to: ["authenticated"], withCheck: sql`( SELECT (count(*) > 0)
   FROM project_member
