import { pgTable, foreignKey, pgPolicy, uuid, timestamp, text, numeric, pgSchema, varchar, uniqueIndex, index, unique, check, jsonb, boolean, smallint, bigserial, inet, json, pgEnum } from "drizzle-orm/pg-core"
...
export const node = pgTable("node", {
	...
	type: text().notNull(),
	xCoord: numeric("x_coord"),
	yCoord: numeric("y_coord"),
	projectId: uuid("project_id"),
}, (table) => [
	foreignKey({
			columns: [table.projectId],
			foreignColumns: [project.id],
			name: "node_project_id_fkey"
		}).onUpdate("cascade").onDelete("cascade"),
	pgPolicy("Allow authenticated users to delete nodes", { as: "permissive", for: "delete", to: ["authenticated"], using: sql`true` }),
	...
	pgPolicy("Allow authenticated users to update nodes", { as: "permissive", for: "update", to: ["authenticated"] }),
]);
...
export const task = pgTable("task", {
	...
	pgPolicy("Allow project members to read tasks", { as: "permissive", for: "select", to: ["authenticated"], using: sql`( SELECT (count(*) > 0)
   FROM project_member
  WHERE ((project_member.project_id = task.project_id) AND (project_member.user_id = ( SELECT auth.uid() AS uid))))` }),
	pgPolicy("Allow project members to edit task", { as: "permissive", for: "update", to: ["authenticated"] }),
]);

export const moduleWebsite = pgTable("module_website", {
	id: uuid().primaryKey().notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	description: text(),
	url: text(),
	healthcheckPath: text("healthcheck_path").default('/api/health').notNull(),
}, (table) => [
...
export const inviteToProject = pgTable("invite_to_project", {
	...
	unique("unique_sender_target").on(table.senderProjectMemberId, table.targetEmail),
  ...
  WHERE (project_member.user_id = ( SELECT auth.uid() AS uid)))`  }),
	...
	pgPolicy("Allow authenticated user to delete their own invites", { as: "permissive", for: "delete", to: ["authenticated"] }),
]);
...
export const projectMember = pgTable("project_member", {
	...
	pgPolicy("Allow authenticated users to read project members by their proj", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(project_id IN ( SELECT my_project_ids.project_id
   FROM my_project_ids() my_project_ids(project_id)))` }),
]);
...
export const notification = pgTable("notification", {
	...
	pgPolicy("Enable users to view their own data only", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(( SELECT auth.uid() AS uid) = target_user_id)` }),
